...
const Card = ({
  ...
  return (
    <div
      onClick={handleClick}
      className="relative w-20 h-20 rounded-lg shadow-md bg-white transform transition-transform duration-100"
    >
      {!isFlipped ? (
        <div className="absolute inset-0 flex items-center justify-center">
          <img src={cardBackImage} alt="Card back" className="w-15 h-15" />
        </div>
      ) : (
        <div className="absolute inset-0 flex items-center justify-center">
          <img
